...
public class TenderServiceimpl implements TenderService {
	...
	// /*~~(Unable to determine parameter type)~~>*/private final
	// controller.tenderItemController tenderItemController;

	// private final MiniTenderprojectApplication miniTenderprojectApplication;

	private final TenderRepository tenderRepository;

	private final TenderCustomRepository tenderCustomRepository;

	public TenderServiceimpl(TenderRepository tenderRepository, TenderCustomRepository tenderCustomRepository) {

		this.tenderRepository = tenderRepository;

		this.tenderCustomRepository = tenderCustomRepository;
	...
	public TenderResponse createTender(TenderRequest request) {
		Tender tender = Tender.builder().tenderNumber(request.getTenderNumber()).title(request.getTitle())
				.description(request.getDescription()).status(request.getStatus())
				.organizationId(request.getOrganizationId()).publishDate(request.getPublishDate())
				.closingDate(request.getClosingDate()).estimatedValue(request.getEstimatedValue()).build();

		Tender saveTender = tenderRepository.save(tender);

		TenderResponse response = TenderResponse.builder().id(saveTender.getId())
				.tenderNumber(saveTender.getTenderNumber()).title(saveTender.getTitle())
				.description(saveTender.getDescription()).status(saveTender.getStatus())
				.organizationId(saveTender.getOrganizationId()).publishDate(saveTender.getPublishDate())
				.closingDate(saveTender.getClosingDate()).estimatedValue(saveTender.getEstimatedValue()).build();
	...
	public TenderResponse getTenderById(Long id) {
		Tender tender = tenderRepository.findById(id)
				.orElseThrow(() -> new RuntimeException("Tender Id Was Not Found :" + id));

		TenderResponse response = TenderResponse.builder().id(tender.getId()).tenderNumber(tender.getTenderNumber())
				.title(tender.getTitle()).description(tender.getDescription()).status(tender.getStatus())
				.organizationId(tender.getOrganizationId()).publishDate(tender.getPublishDate())
				.closingDate(tender.getClosingDate()).estimatedValue(tender.getEstimatedValue()).build();
	...
	/*
	 * 
	 * @Override public List<TenderResponse> getAllTenders() { List<Tender> tender =
	 ...
	 * 
	 * TenderResponse response = TenderResponse.builder() .id(tenders.getId())
	 * .tenderNumber(tenders.getTenderNumber()) .title(tenders.getTitle())
	 * .description(tenders.getDescription()) .status(tenders.getStatus())
	 * .organizationId(tenders.getOrganizationId())
	 * .publishDate(tenders.getPublishDate()) .closingDate(tenders.getClosingDate())
	 * .estimatedValue(tenders.getEstimatedValue()) .build();
	 * 
	 ...
	 * 
	 * return tender.stream() .map(tender -> TenderResponse.builder()
	 * .id(tenders.getId()) .tenderNumber(tenders.getTenderNumber())
	 * .title(tenders.getTitle()) .description(tenders.getDescription())
	 * .status(tenders.getStatus()) .organizationId(tenders.getOrganizationId())
	 * .publishDate(tenders.getPublishDate()) .closingDate(tenders.getClosingDate())
	 * .estimatedValue(tenders.getEstimatedValue()) .build()) .toList();
	 * 
	 * } return responses; }
	 */
	...
	public TenderResponse updateTenderResponse(Long id, TenderRequest request) {
		...
		Tender tender = tenderRepository.findById(id)
				.orElseThrow(() -> new RuntimeException("Tender Id Not Found :" + id));
		...
		TenderResponse response = TenderResponse.builder().id(updateTender.getId())
				.tenderNumber(updateTender.getTenderNumber()).title(updateTender.getTitle())
				.description(updateTender.getDescription()).status(updateTender.getStatus())
				.organizationId(updateTender.getOrganizationId()).publishDate(updateTender.getPublishDate())
				.closingDate(updateTender.getClosingDate()).estimatedValue(request.getEstimatedValue()).build();

		return response;
	}

	private TenderResponse convertToresponsez(Tender tender) {
		return TenderResponse.builder().id(tender.getId()).tenderNumber(tender.getTenderNumber())
				.title(tender.getTitle()).description(tender.getDescription()).status(tender.getStatus())
				.organizationId(tender.getOrganizationId()).publishDate(tender.getPublishDate())
				.closingDate(tender.getClosingDate()).estimatedValue(tender.getEstimatedValue()).build();
	...
	public TenderResponse submitTender(Long id) {
		...
		return TenderResponse.builder().id(saveTender.getId()).tenderNumber(saveTender.getTenderNumber())
				.title(saveTender.getTitle()).description(saveTender.getDescription()).status(saveTender.getStatus())
				.organizationId(saveTender.getOrganizationId()).publishDate(saveTender.getPublishDate())
				.closingDate(saveTender.getClosingDate()).estimatedValue(saveTender.getEstimatedValue()).build();
	}
	...
	public List<TenderItemViewResponse> getTenderItemDtails() {
		...
		return results.stream().map(item -> new TenderItemViewResponse(item.getTenderId(), item.getTenderItemId(),
				item.getTenderName(), item.getPublishDate(), item.getQuantity(), item.getPrice())).toList();
	}
	...
	public List<TenderStatusDetailsResponse> getTenderStatusOrStatus(List<String> statuses) {
		...
		return results.stream()
				.map(tender -> TenderStatusDetailsResponse.builder().tenderId(tender.getTenderId())
						.title(tender.getTitle()).description(tender.getDescribtion()).status(tender.getStatus())
						.tenderItemId(tender.getTenderItemId()).quantity(tender.getQuantity()).unit(tender.getUnit())
						.price(tender.getPrice()).build())
				.toList();
	...
	public TenderStatusResponse updateTenderStatus(long id, TenderStatusRequest request) {
		Tender tender = tenderRepository.findById(id)
				.orElseThrow(() -> new RuntimeException(" Tende Id Not Fount Exception :" + id));
	...
	public List<TenderBidOrganizationReportResponse> gettenderBidOrganizationreport(String status,
		...
		List<TenderBidOrganizationReportProjection> result = tenderRepository.findTenderBidOrganizationReport(status,
				estimatedValue);

		return result.stream()
				.map(r -> new TenderBidOrganizationReportResponse(r.getTenderNumber(), r.getTitle(), r.getStatus(),
						r.getEstimatedValue(), r.getOrganizationName(), r.getDepartment(), r.getTotalItem(),
	...
	public List<Object[]> getAllTenders() {
		...
		List<Object[]> result = tenderCustomRepository.getAllTenders();
	...
	public List<TenderResponse> getTendersByStatus(String status) {
		...
		List<Tender> result = tenderCustomRepository.getTendersByStatus(status);
		
		List<TenderResponse> responseList  = new ArrayList<>();
		
		for(Tender tender : result) {
			
			TenderResponse response = new TenderResponse();
			
			response.setId(tender.getId());
			response.setTenderNumber(tender.getTenderNumber());
			response.setTitle(tender.getTitle());
			response.setDescription(tender.getDescription());
			response.setStatus(tender.getStatus());
			response.setOrganizationId(tender.getOrganizationId());
			response.setPublishDate(tender.getPublishDate());
			response.setClosingDate(tender.getClosingDate());
			response.setEstimatedValue(tender.getEstimatedValue());
	...
	public List<TenderOrganizationResponse1> getTenderByEstimatedValue(BigDecimal estimatedValue) {
		...
		List<Object[]> result = tenderCustomRepository.getTendersByEstimatedValue(estimatedValue);
		
		List<TenderOrganizationResponse1> responseList= new ArrayList<>();
		
		for (Object[] row : result) {
			
			TenderOrganizationResponse1 response = new TenderOrganizationResponse1();
		  
			response.setTenderNumber((String) row[0]);
	        response.setStatus((String) row[1]);
	        response.setOrganizationName((String) row[2]);
